...
#collision_invariant(α::AV, vs::AbstractVelocitySpace1D) = collision_invariant(α, vs.u)

function collision_invariant(α::AV, u, v)
    return @. exp(α[1] + α[2] * u + α[3] * v + α[4] * (u^2 + v^2) / 2)
end

#collision_invariant(α::AV, vs::AbstractVelocitySpace2D) = collision_invariant(α, vs.u, vs.v)

function collision_invariant(α::AV, u, v, w)
    return @. exp(α[1] + α[2] * u + α[3] * v + α[4] * w + α[5] * (u^2 + v^2 + w^2) / 2)
end

#collision_invariant(α::AV, vs::AbstractVelocitySpace3D) = collision_invariant(α, vs.u, vs.v, vs.w)
